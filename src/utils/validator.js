...
localize('zh_CN', {
  ...
  messages: {
    ...
    length: (field, [length, max]) => max ? `${field}长度必须在${length}到${max}之间` : `${field}长度必须为${length}`,
...
// 用户名校验
extend('userName', {
  message: () => '用户名最小长度为6个字符',
  validate: value => /^\S{6,10}$/.test(value)
})
// 密码校验
extend('password', {
  message: () => '密码至少6位包含大写小写及数字',
  validate: value => /^(?=.*?[A-Z])(?=.*?[a-z])(?=.*?[0-9]).{6,16}$/.test(value)
})
// 手机号码验证
extend('mobile', {
  message: () => '请输入正确的手机号码',
